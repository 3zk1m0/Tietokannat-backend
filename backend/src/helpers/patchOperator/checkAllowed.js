...
export default function checkAllowed(ctx, body, allowed) {
  try {
    body.forEach((element) => {
      ...
      } else if (typeof allowed.value[allowed.path.indexOf(element.path)] !== typeof element.value) {
        ctx.throw(400, `value type ${typeof element.value} is not allowed`);
      }
    });
  } catch (error) {
    ctx.throw(400, 'Body not valid');
  }
}
