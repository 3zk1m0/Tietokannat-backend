...
export default async function patchLoans(ctx) {
  ...
  try {
    ...
    if (data[0].loanReceiver_uuid === null) {
      await conn.execute(`
        UPDATE loans
        SET id = uuid_to_bin('${data[0].uuid}'), loaningTime = '${data[0].loaningTime}', dueDate = '${data[0].dueDate}', loansState = '${data[0].loansState}', device_id = uuid_to_bin('${data[0].device_uuid}'), customer_id = uuid_to_bin('${data[0].customer_uuid}'), loanGiver_id = uuid_to_bin('${data[0].loanGiver_uuid}')
        WHERE uuid = '${id}';
    `);
    } else {
      await conn.execute(`
